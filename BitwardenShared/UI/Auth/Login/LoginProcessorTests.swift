...
class LoginProcessorTests: BitwardenTestCase { // swiftlint:disable:this type_body_length
    ...
    /// A second master-password submission is ignored while the first login request is suspended.
    @MainActor
    func test_perform_loginWithMasterPasswordPressed_doubleSubmit() async throws {
        subject.state.username = "email@example.com"
        subject.state.masterPassword = "Password1234!"
        authRepository.unlockWithPasswordResult = .success(())
        authRepository.activeAccount = .fixture()

        let loginStarted = expectation(description: "Login request started")
        var loginContinuation: CheckedContinuation<Void, Never>?
        authService.loginWithMasterPasswordHandler = {
        ...
        XCTAssertTrue(subject.state.isLoggingIn)
        XCTAssertTrue(coordinator.isLoadingOverlayShowing)
        await subject.perform(.loginWithMasterPasswordPressed)
        XCTAssertEqual(authService.loginWithMasterPasswordCallCount, 1)
        ...
        XCTAssertEqual(authRepository.unlockVaultPassword, "Password1234!")
        XCTAssertFalse(coordinator.isLoadingOverlayShowing)
        XCTAssertFalse(subject.state.isLoggingIn)
    }

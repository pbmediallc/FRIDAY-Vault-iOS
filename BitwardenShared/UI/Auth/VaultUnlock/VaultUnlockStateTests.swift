...
class VaultUnlockStateTests: BitwardenTestCase {
    ...
    /// New vault unlock state is idle until the processor begins an unlock attempt.
    func test_init_isUnlockingFalse() {
        let subject = VaultUnlockState(account: .fixture())

        XCTAssertFalse(subject.isUnlocking)
    }
}

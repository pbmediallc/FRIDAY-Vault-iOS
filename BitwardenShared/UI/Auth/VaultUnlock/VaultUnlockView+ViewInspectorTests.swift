...
class VaultUnlockViewTests: BitwardenTestCase {
    ...
    /// The app extension keeps the compact unlock layout without the full app brand header.
    @MainActor
    func test_extensionUnlock_usesCompactLayout() {
        processor.state.isInAppExtension = true

        XCTAssertThrowsError(try subject.inspect().find(text: "F.R.I.D.A.Y. VAULT"))
        XCTAssertNoThrow(try subject.inspect().find(button: Localizations.unlock))
    }
    ...
    /// Unlock controls are disabled while the processor owns an active unlock attempt.
    @MainActor
    func test_unlocking_disablesUnlockButton() throws {
        processor.state.isUnlocking = true

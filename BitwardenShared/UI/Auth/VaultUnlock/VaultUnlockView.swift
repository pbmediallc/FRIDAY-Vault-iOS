...
struct VaultUnlockView: View {
    ...
    /// The password, PIN, and biometric unlock controls.
        ...
        VStack(spacing: 24) {
            ...
            VStack(spacing: 12) {
                ...
                if store.state.shouldShowPasswordOrPinFields {
                    ...
                    .disabled(store.state.isUnlocking)
                }
            }
        }
        .disabled(store.state.isUnlocking)
    ...
    @ViewBuilder private var biometricAuthButton: some View {
        if case let .available(biometryType, true) = store.state.biometricUnlockStatus {
            AsyncButton {
                await store.perform(.unlockVaultWithBiometrics)
            } label: {
    ...
    @ViewBuilder private var textField: some View {
        ...
        case .password:
            ...
            .submitLabel(.go)
            .onSubmit {
                guard !store.state.isUnlocking else { return }
                Task {
                    await store.perform(.unlockVault)
                }
            }

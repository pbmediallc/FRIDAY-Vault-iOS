...
class VaultUnlockProcessorTests: BitwardenTestCase { // swiftlint:disable:this type_body_length
    ...
    /// `perform(.appeared)` automatically unlocks with biometrics through the guarded unlock path.
    @MainActor
    func test_perform_appeared_loadData_unlockWithBiometrics_success() async throws {
        stateService.activeAccount = .fixture()
    ...
    /// Automatic biometric unlock applies the same extension KDF preflight as a manual attempt.
    @MainActor
    func test_perform_appeared_loadData_unlockWithBiometrics_extensionKdfWarning() async throws {
    ...
    /// A second password or biometric submission is ignored while an unlock attempt is active.
    @MainActor
    func test_perform_unlock_doubleSubmit() async {
        subject.state.isUnlocking = true
        subject.state.masterPassword = "password"
        biometricsRepository.getBiometricUnlockStatusReturnValue = .available(.faceID, enabled: true)
        ...
        XCTAssertNil(authRepository.unlockVaultPassword)
        XCTAssertFalse(authRepository.unlockVaultWithBiometricsCalled)
        XCTAssertTrue(coordinator.loadingOverlaysShown.isEmpty)
        XCTAssertTrue(subject.state.isUnlocking)
    }

    /// A second unlock submission is ignored while the first extension KDF preflight is suspended.
    @MainActor
    func test_perform_unlock_doubleSubmitDuringExtensionKdfPreflight() async {
        appExtensionDelegate.isInAppExtension = true
        subject.state.masterPassword = "password"
        biometricsRepository.getBiometricUnlockStatusReturnValue = .available(.faceID, enabled: true)

        let preflightStarted = expectation(description: "Extension KDF preflight started")
        var accountContinuation: CheckedContinuation<Account, Never>?
        stateService.getAccountHandler = { _ in
        ...
        accountContinuation?.resume(
            returning: .fixture(profile: .fixture(kdfMemory: 65, kdfType: .argon2id)),
        )

...
struct AboutView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            FridayVaultBrandHeader(
                title: AboutState.productName,
                subtitle: AboutState.publisherName,
                motionEnabled: false,
            )
            .padding(.vertical, 8)
    ...
    private var miscSection: some View {
        ContentBlock(dividerLeadingPadding: 16) {
            externalLinkRow(
                AboutState.upstreamLinkTitle(Localizations.bitwardenHelpCenter),
                action: .helpCenterTapped,
            )

            externalLinkRow(AboutState.webVaultTitle, action: .webVaultTapped)

            externalLinkRow(
                AboutState.upstreamLinkTitle(Localizations.learnOrg),
                action: .learnAboutOrganizationsTapped,
            )

            externalLinkRow(AboutState.fridaySourceTitle, action: .fridaySourceTapped)

            externalLinkRow(AboutState.upstreamSourceTitle, action: .upstreamSourceTapped)

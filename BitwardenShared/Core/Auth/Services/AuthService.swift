...
/// The auth connector flow that an `ASWebAuthenticationSession` is being created for.
///
...
protocol AuthService {
    ...
    /// Provides a web authentication session configured with the upstream-compatible
    /// `bitwarden://` callback for the given connector flow.
    ///
    /// - Parameters:
...
class DefaultAuthService: AuthService { // swiftlint:disable:this type_body_length
    ...
    /// The single sign on callback URL for this application. Must stay in sync with the callback
    /// the `ASWebAuthenticationSession` is configured to match.
    ...
    func webAuthenticationSession(
        url: URL,
        callbackKind _: AuthWebSessionCallbackKind,
        completionHandler: @escaping ASWebAuthenticationSession.CompletionHandler,
    ) -> ASWebAuthenticationSession {
        ASWebAuthenticationSession(
            url: url,
    ...
    /// - Returns: e.g. `bitwarden://sso-callback`.
    ///
    private func callbackUrl(for kind: AuthWebSessionCallbackKind) -> String {
        "\(callbackUrlScheme)://\(kind.callbackPath)"
    }
    ...
    private func getIdentityTokenResponse( // swiftlint:disable:this function_body_length
        ...
        if let authenticationMethod {
            ...
            request = IdentityTokenRequestModel(
                ...
                deeplinkScheme: callbackUrlScheme,

...
class AuthServiceTests: BitwardenTestCase { // swiftlint:disable:this type_body_length
    ...
    /// `generateSingleSignOnUrl(from:)` uses the upstream custom-scheme callback for Cloud (US).
    func test_generateSingleSignOnUrl_unitedStatesUsesCustomScheme() async throws {
        client.result = .httpSuccess(testData: .preValidateSingleSignOn)
        clientService.mockGenerators.passwordReturnValue = "PASSWORD"
    ...
    /// `generateSingleSignOnUrl(from:)` uses the upstream custom-scheme callback for Cloud (EU).
    func test_generateSingleSignOnUrl_europeUsesCustomScheme() async throws {
        client.result = .httpSuccess(testData: .preValidateSingleSignOn)
        clientService.mockGenerators.passwordReturnValue = "PASSWORD"
    ...
    /// `generateSingleSignOnUrl(from:)` uses the upstream custom-scheme callback internally.
    func test_generateSingleSignOnUrl_internalUsesCustomScheme() async throws {
        client.result = .httpSuccess(testData: .preValidateSingleSignOn)
        clientService.mockGenerators.passwordReturnValue = "PASSWORD"
